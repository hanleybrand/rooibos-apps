...
def get_install_dir():
    ...
    while not os.path.exists(os.path.join(install_dir, 'rooibos', 'settings.py')):
        up_dir = os.path.normpath(os.path.join(install_dir, '..'))
        if up_dir == install_dir:
            break
        install_dir = up_dir
    return install_dir
...
os.environ["ROOIBOS_ADDITIONAL_SETTINGS"] = 'apps.jmutube.settings_local_standalone'
